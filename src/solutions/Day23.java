...
public class Day23 {
    ...
    private static int area(Map<Point, Elf> elves) {
        int[] mm = elves.keySet().stream().collect(() -> new int[]{Integer.MAX_VALUE, 0, Integer.MAX_VALUE, 0},
                (e, e2) -> {
                    e[0] = Math.min(e[0], e2.x);
                    e[1] = Math.max(e[1], e2.x);
                    e[2] = Math.min(e[2], e2.y);
                    e[3] = Math.max(e[3], e2.y);
                }, (e, e2) -> {
                });
    ...
    private static int[] solve(Map<Point, Elf> elves, LinkedList<Direction> dirs, int rounds) {
        ...
        for (int r = 1; r <= rounds; r++) {
            ...
            proposed.entrySet().stream().filter(e -> e.getValue().size() == 1).forEach(e -> {
                moved.set(true);
                elves.remove(e.getValue().get(0).pos);
                elves.put(e.getKey(), e.getValue().get(0));
            });
