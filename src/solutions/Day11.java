...
public class Day11 {
    ...
    public static Object preprocess(String input, int star) {
        List<Monkey> newinput = Arrays.stream(input.split("\n\n"))
            .map(s -> { Object[][] ss = s.lines().map(sss -> // Get 2d Object arr -> row is line, column is str + digit
                ...
                return new Monkey(new ArrayList<>(List.of((Long)ss[0][1], 0L)), // Monkey num
                        ...
                        i -> { // Func to apply on inspection
                            ...
                            if (((String)ss[2][0]).contains("*")) return n * i; return n + i;
                        },
    ...
    public static Object solve(List<Monkey> input, int rounds, TwoParameterFunctionByReturn f) {
        ...
        IntStream.range(0, rounds).forEach(i -> input.forEach(m -> {
            ...
            m.wl.forEach(item -> { Long a = f.apply(m.op.apply(item), div); // For each item, find new value
            input.get((int)(long)m.t.get(a % m.t.get(0) == 0 ? 1 : 2)).wl.add(a); }); m.wl.clear(); // Move and clear
        }));
        return input.stream().map(m -> m.i.get(1)).sorted(Comparator.reverseOrder())
                .limit(2).reduce(1L, (acc, i) -> acc * i);
    }
    public static Object solveStar1(List<Monkey> input) { return solve(input, 20, (a, b) -> a / 3); }
    public static Object solveStar2(List<Monkey> input) { return solve(input, 10000, (a, b) -> a % b); }
}
