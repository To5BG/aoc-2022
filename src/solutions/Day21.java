...
public class Day21 {
    public record Monkey(Operation op, String[] names, Monkey[] monkes, Long v) {
        public long getValue() {
            return (v == null) ? op.apply(monkes[0].getValue(), monkes[2].getValue()) : v;
        }
        ...
        public void linkMonkeys(final Map<String, Monkey> allMonkeys) {
            for (int i : new int[]{0, 2}) {
                monkes[i] = allMonkeys.get(names[i]);
                if (monkes[i] != null) monkes[i].monkes[1] = this;
            }
        }
        ...
        private Monkey getOtherChild(final Monkey monkey) {
            return monkes[monkey == monkes[0] ? 2 : 0];
        }
    }
    ...
    public enum Operation {
        ...
        public long applyInverse(long in, long out, boolean isLeft) {
            return switch (this) {
                case ADD -> out - in;
                case MULTIPLY -> out / in;
                case SUBTRACT -> isLeft ? in + out : in - out;
                case DIVIDE -> isLeft ? in * out : in / out;
            };
        }
    }
    ...
    public static Monkey createMonke(String str) {
        Long v = null;
        Operation op = null;
        String[] names = new String[]{"", str.substring(0, 4), ""};
        if (str.length() == 17) {
            op = Operation.save(str.charAt(11));
            names[0] = str.substring(6, 10);
            names[2] = str.substring(13);
